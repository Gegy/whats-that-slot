...
public final class SlotQueryGenerator {
    ...
    public SlotQueryGenerator(ContainerScreen<?> screen, Predicate<ItemStack> filter) {
    ...
    public SlotQuery advance(BooleanSupplier shouldContinue) {
        ...
        while (shouldContinue.getAsBoolean()) {
            ...
            if (state instanceof Ready) {
                return this.results = ((Ready) state).results;
            }
        }
    ...
    private static final class MatchingInventory implements State {
        ...
        private MatchingInventory(ContainerScreen<?> screen, Predicate<ItemStack> filter) {
            this.filter = filter;
    ...
    private static final class MatchingGlobal implements State {
        ...
        private List<QueriedItem> buildInventoryResults() {
            List<QueriedItem> inventoryItems = new ObjectArrayList<>(this.inventoryMatches.size());
    ...
    private static final class Ready implements State {
        private final SlotQuery results;

        private Ready(SlotQuery results) {
